...
from amaris.connectors.sqla.models import AnnotationDatasource, SqlaTable
from amaris.exceptions import SupersetException, SupersetSecurityException
from amaris.forms import CsvToDatabaseForm
from amaris.jinja_context import get_template_processor
from amaris.legacy import cast_form_data
import amaris.models.core as models
...
class Superset(BaseSupersetView):
    ...
    def dashboard(self, dashboard_id):
        ...
        if dashboard_view == 'v2':
            entry = 'dashboard'
            template = 'amaris/dashboard.html'
        else:
            entry = 'dashboard_deprecated'
            template = 'amaris/dashboard_v1_deprecated.html'
...
def panoramix(url):  # noqa
    return redirect(request.full_path.replace('panoramix', 'amaris'))


@app.route('/<regex("caravel\/.*"):url>')
def caravel(url):  # noqa
    return redirect(request.full_path.replace('caravel', 'amaris'))
