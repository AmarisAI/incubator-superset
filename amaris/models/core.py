...
from amaris import app, db, db_engine_specs, security_manager, utils
from amaris.connectors.connector_registry import ConnectorRegistry
from amaris.models.helpers import AuditMixinNullable, ImportMixin, set_perm
from amaris.viz import viz_types
install_aliases()
...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    def get_explore_url(self, base_url='/amaris/explore', overrides=None):
    ...
    def explore_json_url(self):
        """Defines the url to access the slice"""
        return self.get_explore_url('/amaris/explore_json')
...
class Dashboard(Model, AuditMixinNullable, ImportMixin):
            ...
            if default_filters and default_filters != '{}':
                try:
                    if json.loads(default_filters):
                        ...
                        return '/amaris/dashboard/{}/?preselect_filters={}'.format(
                            self.slug or self.id, filters)
                except Exception:
                    pass
        return '/amaris/dashboard/{}/'.format(self.slug or self.id)
    ...
    def import_obj(cls, dashboard_to_import, import_time=None):
        """Imports the dashboard from the object to the database.
         ...
         to import/export dashboards between multiple amaris instances.
...
class Database(Model, AuditMixinNullable, ImportMixin):
    ...
    @property
    def sql_url(self):
        return '/amaris/sql/{}/'.format(self.id)
    ...
    def roles_with_datasource(self):
        ...
        for r in pv.role:
            if r.name in self.ROLES_BLACKLIST:
                continue
            url = (
                '/amaris/approve?datasource_type={self.datasource_type}&'
                'datasource_id={self.datasource_id}&'
